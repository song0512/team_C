...
public class PlayGame implements ActionListener{
	...
	void doYot()
	{
		if(control==1) {
			...
			doYotchangeBtncolor();//UI 버튼 색깔 변경
	...
	void dogYot()
	{
		if(control==1) {
			...
			//result=Yoot.throwing();//던지기 버튼 클릭
			board.printResult(result);//던진 결과 화면에 출력
			dogYotchangeBtncolor();//UI 버튼 색깔 변경
	...
	void girlYot()
	{
		if(control==1) {
			...
			//result=Yoot.throwing();//던지기 버튼 클릭
			board.printResult(result);//던진 결과 화면에 출력
			girlYotchangeBtncolor();//UI 버튼 색깔 변경
	...
	void againYot()
	{
		if(control==1) {
			...
			result=Yoot.throwing();//던지기 버튼 클릭
			board.printResult(result);//던진 결과 화면에 출력
			againYotchangeBtncolor();//UI 버튼 색깔 변경
	...
	//도 확정 버튼을 누르면 색깔 파랑색으로
	void doYotchangeBtncolor() {
		if(nowPlayer.getPieces().size()>0)
		{
			board.buttonColor("clickBoardON");//윷판 버튼 노랑
			board.buttonColor("throwBtnOFF");
		}
		else
		{
			board.buttonColor("clickBoardOFF");//윷판 버튼 파랑
			board.buttonColor("throwBtnOFF");
		}
		if(nowPlayer.getPieceNum()>0)
		{
			board.buttonColor("newPieceBtnON");//새로운말 노랑
		}
		else
		{
			board.buttonColor("newPieceBtnOFF");//새로운말 파랑
		}
	}
	//개 확정 버튼을 누르면 색깔 파랑색으로
